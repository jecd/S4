...
def colorDoc(documentID = 'd7pp4q-027', colorLabels = [], my_dpi = 220.53, colorBy = 'confidence'):
    sql_command = "SELECT * FROM {} WHERE page_ark = '{}';".format("rtesseract_words",documentID)
    ...
    for idx,word,c in zip(range(len(df)),df.text,colorLabels):
        ...
        ax.annotate(word,start, color='k', weight='bold', 
                    fontsize=height/6, ha='left', va='bottom')
